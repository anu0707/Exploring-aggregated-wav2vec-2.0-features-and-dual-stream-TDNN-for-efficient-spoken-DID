...
class StatPooling(nn.Module):
    ...
    def forward(self, filename):
        ...
        for i, layer_features in enumerate(features):
            ...
            mean = torch.mean(layer_features,1)
            var = torch.var(layer_features,1)
            stat_pooling = torch.cat((mean,var),1)
            #print(f'stat pooling {stat_pooling.shape}')
...
txtfl = open('/mnt/Disk12TB/DID/slt/large_wofinetune/stat_pool_train.txt', 'w')
...
for e in range(n_epoch): # repeat for n_epoch
    ...
    path = "/mnt/Disk12TB/DID/slt/large_wofinetune/stat_pool/e_"+str(e+1)+".pth"
