...
class GlobalBlock(nn.Module):
    ...
    """
    def __init__(self, dim, T=300, dropout=0.2, K=4):
        super().__init__()
...
class DS_TDNN(nn.Module):
    ...
    def forward_(self, x, aug=True):    
        ...
        x = self.fc8(x)
        x = self.fc7(x)
        print(f'x shape in forward_ {x.shape}')
        return x   
...
#model.load_state_dict(torch.load('/mnt/Disk12TB/DID/slt/ds-tdnn/model3/e_115.pth'))
txtfl = open('/mnt/Disk12TB/DID/slt/ds-tdnn/trainModel4.txt', 'w')
for e in range(200): # repeat for n_epoch
    ...
    #print("In training generator")
        #XX1 = Xdata1.reshape(Xdata1.shape[1:])
        #YY1 = Y1.reshape(Y1.shape[1:])
        #YY2 = Y2.reshape(Y2.shape[1:])
        ...
        X1 = Variable(Xdata1,requires_grad=False).cuda()
        Y1 = Variable(Y1,requires_grad=False).cuda()
 # setting model gradient to zero before calculating gradient
        ...
        print("x-vec-bnf.py: Epoch = ",e+1,"  completed files  "+str(i)+" Loss= %.7f"%(cost/i))
